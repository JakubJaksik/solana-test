...
impl SlotHashCache {
    ...
    /// Record (or overwrite) the latest entry hash seen for `slot`. Observer
    /// calls this on every merged entry; the last write before the slot is
    /// sealed wins, which is what we want for `last_entry_hash`.
    pub fn update(&self, slot: u64, entry_hash: Hash) {
...
mod tests {
    ...
    #[test]
    fn update_overwrites_for_same_slot() {
        let cache = SlotHashCache::new(64);
    ...
    fn lookup_falls_back_when_predecessor_skipped() {
        ...
        let (src, found) = cache.lookup_recent_blockhash(100, 5).unwrap();
        assert_eq!(src, 98);
        assert_eq!(found, h);
    }
    ...
    fn lookup_returns_none_when_beyond_lookback() {
        let cache = SlotHashCache::new(64);
        cache.update(90, Hash::new_unique());
    ...
    fn eviction_drops_old_slots() {
        ...
        for s in 100..120u64 {
            cache.update(s, Hash::new_unique());
        }
